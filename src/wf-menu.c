...
int MenuConfDataEntryCb(void *m) {
	...
	if (error || !ip->addr) {
		item[2].caption.string = (char*)strIpAuto;
		item[2].caption.length = sizeof(strIpAuto) - 1;
	} else {
		item[2].caption.string = (char*)strIpManual;
		item[2].caption.length = sizeof(strIpManual) - 1;
	}
	i = 0;
